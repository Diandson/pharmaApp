...
export class AssuranceComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.assuranceService
      ...
      .subscribe({
        next: (res: HttpResponse<IAssurance[]>) => {
  ...
  trackId(index: number, item: IAssurance): number {
    return item.id!;
  }

  delete(assurance: IAssurance): void {
    const modalRef = this.modalService.open(AssuranceDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IAssurance[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
