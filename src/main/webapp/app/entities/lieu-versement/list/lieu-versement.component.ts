...
@Component({
  ...
  templateUrl: './lieu-versement.component.html',
})
export class LieuVersementComponent implements OnInit {
  lieuVersements?: ILieuVersement[];
  isLoading = false;
  totalItems = 0;
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.lieuVersementService
      ...
      .subscribe({
        next: (res: HttpResponse<ILieuVersement[]>) => {
  ...
  trackId(index: number, item: ILieuVersement): number {
    return item.id!;
  }

  delete(lieuVersement: ILieuVersement): void {
    const modalRef = this.modalService.open(LieuVersementDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: ILieuVersement[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/lieu-versement'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
