...
export class MessageService {
  ...
  protected convertDateFromClient(message: IMessage): IMessage {
    return Object.assign({}, message, {
      dateMessage: message.dateMessage?.isValid() ? message.dateMessage.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateMessage = res.body.dateMessage ? dayjs(res.body.dateMessage) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((message: IMessage) => {
        message.dateMessage = message.dateMessage ? dayjs(message.dateMessage) : undefined;
      });
    }
    return res;
  }
